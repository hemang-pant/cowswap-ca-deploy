...
import { getSupplyVal } from 'cow-react';
  ...
  return (
    <MainContainer $display={$display}>
      ...
      <Root defaultValue={['sources']} collapsible>
        <AccordionWrapper key="sources" value="sources">
          <Header>
            ...
            <HeaderRight>
              <TotalFees>
                {getReadableNumber(new Decimal(intent.destination.amount).add(balances.find((balance) => balance.symbol === intent?.token?.symbol)?.breakdown.find((breakdown) => breakdown.chain.id === intent?.destination?.chainID)?.balance!).toString())}{' '}
                {intent?.token?.symbol}
              </TotalFees>
              {intent?.token?.symbol && rates?.[intent.token.symbol] && (
                <TotalAtDestination>
                  ~
                  {(Number(
                    Decimal(intent.destination.amount).add(balances.find((balance) => balance.symbol === intent?.token?.symbol)?.breakdown.find((breakdown) => breakdown.chain.id === intent?.destination?.chainID)?.balance!)
                  ) / Number(rates[intent.token.symbol])).toFixed(
                    2
                  )}{' '}
                  USD
                </TotalAtDestination>
      ...
      <Header>
        ...
        <HeaderRight>
          <TotalFeesValue>
            {getReadableNumber(new Decimal(intent.destination.amount).add(intent.fees.total).add(balances.find((balance) => balance.symbol === intent?.token?.symbol)?.breakdown.find((breakdown) => breakdown.chain.id === intent?.destination?.chainID)?.balance!).toString())}{' '}
                {intent?.token?.symbol}
          </TotalFeesValue>

          {rates?.[intent?.token?.symbol] && (
            <TotalAtDestinationValue>
              ~{(Number(new Decimal(intent.destination.amount).add(intent.fees.total).add(balances.find((balance) => balance.symbol === intent?.token?.symbol)?.breakdown.find((breakdown) => breakdown.chain.id === intent?.destination?.chainID)?.balance!) || 0) / Number(rates[intent?.token?.symbol])).toFixed(2)}{' '}
              USD
            </TotalAtDestinationValue>
          )}
        </HeaderRight>
